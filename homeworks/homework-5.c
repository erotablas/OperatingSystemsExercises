...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if ((token = strtok(commandline, DELIM)) == NULL) {                    //restituisce una lista di stringhe delimitate
        ...
        do {
            ...
            if (token[0] == '<') {
                if (path_stdin == NULL) {
                    ...
                    if (!access(path_stdin, R_OK)) {        //controlla se il percorso e' disponibile in lettura
                        fprintf(stderr, "impossibile accedere in lettura al file specificato come standard input!\n");
                        free(path_stdin);   //deallocazione
                        continue;
                    }
                } else {
            ...
            args[i++] = strdup(token);
        } while ((token = strtok(NULL, DELIM)) != NULL);       //indecifrabile (probabilmente pass alla prossima stringa dopo ogni delimitatore)
        args[i] = NULL;
